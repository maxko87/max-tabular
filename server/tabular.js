...
Meteor.publish("tabular_getInfo", function(tableName, selector, sort, skip, limit) {
  ...
  console.log(Date(), "Starting pub...");

  var filteredCursor = table.collection.find(selector, findOptions);

  console.log(Date(), "Got filteredCursor...");
  console.log(Date(), filteredCursor.count())

  // var filteredRecordIds = _.map(filteredCursor.fetch(), function(doc){  
  var filteredRecordIds = filteredCursor.map(function (doc) {
    return doc._id;
  });

  console.log(Date(), "Got filteredRecordIds...");

  // console.log(Date(), "ids:", filteredRecordIds);

  var countCursor = table.collection.find(selector, {fields: {_id: 1}});
  console.log(Date(), "countCursor:", countCursor.count());

  var recordReady = false;
  function updateRecords() {
    console.log(Date(), "Calling updateRecords()");
  ...
  console.log(Date(), "Before updateRecords()")
  updateRecords();
  console.log(Date(), "After updateRecords()")
  self.ready();
  console.log(Date(), "After self.ready()")
  Tracker.flush()

  // Handle docs being added or removed from the result set.
  var initializing1 = true;
  console.log(Date(), "About to set up observeChanges for handle1")
  ...
  console.log(Date(), "About to set up observeChanges for handle2")
  var handle2 = countCursor.observeChanges({
    added: function () {
      if (initializing2) {
        return;
      }
      console.log("added callback on handle2");
      updateRecords();
    },
    removed: function () {
      console.log("removed callback on handle2");
      updateRecords();
    }
  });
  initializing2 = false;
  console.log(Date(), "Done setting up all handles")
